    ...
    */
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if (!isRefreshing && !isRefreshControlAnimating) {
            centerGear.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi/2) * pullRatio)
...
private extension GearRefreshControl {
    ...
    func animateRefreshView() {
        ...
        UIView.animate(withDuration: 0.3, delay: 0, options: .curveLinear, animations: {
            self.centerGear.transform = self.centerGear.transform.rotated(by: CGFloat(Double.pi/2))
            self.topGear.transform = self.topGear.transform.rotated(by: -CGFloat(Double.pi/2))
            self.rightGear.transform = self.rightGear.transform.rotated(by: CGFloat(Double.pi/2))
            self.bottomGear.transform = self.bottomGear.transform.rotated(by: -CGFloat(Double.pi/2))
            self.leftGear.transform = self.leftGear.transform.rotated(by: CGFloat(Double.pi/2))
